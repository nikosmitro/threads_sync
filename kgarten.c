...
struct kgarten_struct {
        ...
        int free;       /*how many kids can enter*/
        /* ... */
...
void child_enter(struct thread_info_struct *thr)
{
        ...
        while ( thr->kg->free == 0 ) {          /* a new child cant enter so wait*/
                pthread_cond_wait(&thr->kg->child_in, &thr->kg->mutex);
        }

        /*woke up, now there is space to enter a child*/
        ++(thr->kg->vc);
        --(thr->kg->free);      /*reduce the amount of kid that can enter cuz a kid came*/
...
void child_exit(struct thread_info_struct *thr)
{
        ...
        /* there is no restriction for the kids that want to leave*/
        --(thr->kg->vc);
        ++(thr->kg->free);      /*increase the amount of kid that can enter cuz a kid left*/

        if ( ((thr->kg->vt-1) * (thr->kg->ratio)) >= thr->kg->vc ) pthread_cond_broadcast(&thr->kg->teacher_out);     /*one extra teacher, wake up threads to leave*/
        if ( thr->kg->free > 0 ) pthread_cond_broadcast(&thr->kg->child_in);     /*available space for a kid ,wake up threads to enter*/
...
void teacher_enter(struct thread_info_struct *thr)
{
        ...
        /*there is no restriction for a teacher to enter*/
        ++(thr->kg->vt);
        thr->kg->free += thr->kg->ratio;        /*increase the amount of kid that can enter cuz a teacher came*/
...
void teacher_exit(struct thread_info_struct *thr)
{
        ...
        pthread_mutex_lock(&thr->kg->mutex);
        while (((thr->kg->vt-1) * (thr->kg->ratio))< (thr->kg->vc)) {   /*there is no extra teacher so no teacher cant leave for now, so wait*/
        pthread_cond_wait(&thr->kg->teacher_out, &thr->kg->mutex );
        }
        /*woke up, now there is an extra teacher so he can leave*/
        --(thr->kg->vt);
        thr->kg->free -= thr->kg->ratio;        /*reduce the amount of kid that can enter cuz a teacher left*/
